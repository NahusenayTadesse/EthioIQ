<script lang="ts">
  ...
  import type { ExistingParentSchema } from '$lib/server/zodschema';
  ...
  let { data, id, action="?/addExistingParent", items } : { data : SuperValidated<Infer<ExistingParentSchema>> } = $props();
...
{/snippet}
 ...
 <form {action} method="post" class="w-full p-2 flex flex-col gap-4" use:enhance>
     ...
     <div class="flex w-full flex-col justify-start">
		<label for="parentId" class={label}>Choose from a list of Parents:</label>

     <ComboboxComp bind:value={$form.parent} name="parent" {items} />
    
		{#if $errors.parent}<span class="text-red-500">{$errors.parent}</span>{/if}
	</div>
